...
function Cart() {
  ...
  const handleClick = (event) => {
    if (products) {
      ...
      if (products.length) {
        ...
        axios
          .post("https://logansbackyardapi.onrender.com/api/cart/", url)
          .then((products) => {
            setproduct(products.data);
            setTotal(
              products.data.map((items) => items.Price).reduce((a, b) => a + b)
            );
            setLoading(false);
            setIsEmpty(false);
          })
  ...
  return (
    <>
      ...
      <div
        className={`fixed ${showcart} inset-0 overflow-hidden`}
