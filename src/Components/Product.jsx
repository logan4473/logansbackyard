...
function Product(props) {
    ...
    const fetchData = async () => {
      try {
        ...
        const { data } = await axios.get(
          `https://logansbackyardapi.onrender.com/api/product/${ID}`
        );
        const similar = await axios.get(
          `https://logansbackyardapi.onrender.com/api/products/query/?Category=${data.Category}`
        );
        setLoading(false);
        setProduct(data);
  ...
  const handleClick = (event) => {
    if (user) {
      ...
      axios
        .post("https://logansbackyardapi.onrender.com/api/cart/update", url)
        .then(({ data }) => {
          ...
          <div className="md:flex md:items-center">
            ...
            <div className="w-full max-w-lg mx-auto mt-5 md:ml-8 md:mt-0 md:w-1/2">
              ...
              <div className="flex items-center mt-6">
                <Link
                  to={{ pathname: "/payment", product: [product], qty: count }}
                >
                  <span className="px-8 py-2 bg-indigo-600 text-white text-sm font-medium rounded hover:bg-indigo-500 focus:outline-none focus:bg-indigo-500">
                    Order Now
                  </span>
                </Link>
                <button
