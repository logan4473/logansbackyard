...
function Payment({ location }) {
  ...
  const handleSubmit = (event) => {
    ...
    product.forEach((val) => {
      ...
      axios
        .post("https://logansbackyardapi.onrender.com/api/payment", url)
        .then(({ data }) => {
  ...
  return (
    <div
      className="fixed flex justify-center bg-cover h-full w-full"
      style={
        width > 768
    ...
    >
      <form
        className="fixed max-w-xl m-4 p-10 bg-white bg-opacity-50 rounded shadow-xl"
      ...
      >
        ...
        <div className="mt-2">
          ...
          <input
            className="w-full px-2 py-2 text-gray-700 bg-white rounded leading-tight focus:outline-none focus:shadow-outline"
            name="name"
            type="text"
            required
